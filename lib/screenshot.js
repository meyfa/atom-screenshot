...
export default {
  ...
  takeScreenshot () {
    ...
    this.capturer.captureRange(window, editor, screenshotRange, (captureErr, result) => {
      ...
      this.showSaveDialog(window, editor, result)
    })
  },
  ...
  showSaveDialog (window, editor, captureResult) {
    ...
    remote.dialog.showSaveDialog(window, {
      ...
      // if a location was selected, save to there
      if (filename) {
        this.saveCaptureResult(captureResult, filename)
      }
    })
  },

  saveCaptureResult (captureResult, filename) {
    captureResult.saveAs(filename, (saveErr) => {
      // error handling
      if (saveErr) {
